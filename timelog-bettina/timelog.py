...
class TimeLogEntryBettina(db.Model):
...
if "action" in form:
    ...
    result = db.GqlQuery("SELECT * FROM TimeLogEntryBettina WHERE time <= :1 AND time > :2", time, time - 30*24*60*60)
